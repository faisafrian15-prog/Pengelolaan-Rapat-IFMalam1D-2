...
function ambilDataAdmin($koneksi, $username)
{
    try {
        $stmt = $koneksi->prepare(
            "SELECT username, jurusan, email 
             FROM users 
             WHERE username = ? 
             LIMIT 1"
        );
...
$userData = ambilDataAdmin($koneksi, $username);
?>
...
<footer class="footer-custom text-center py-3 border-top mt-auto">
  <div class="text-muted small">&copy; 2025 - Profil Admin Pengelolaan Rapat</div>
</footer>
